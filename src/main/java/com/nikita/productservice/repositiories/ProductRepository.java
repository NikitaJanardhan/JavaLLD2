...
Product findByPriceBetweenAndTitleLessThanEqual(double greaterThanEqualPrice,
                                                double lessThanEqualPrice, String titleLessThan);
...
//    @Query("select * from users join product join category on product.c_id = category.id")
   ...
   // @Query("select p from Product p where p.id=:id and p.category.name=:catagoryName")
    //public List<Product>getByIdAndtitle(Long id,String catagoryName);
